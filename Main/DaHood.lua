--[[
here to settle stuff, this is not skidded lmao this is just a cracked version of dimag x but rebranded into a voidz script for da hood. i hate dimag so much, he is racist + a idiot lol

this is open source, cause why would i obfuscate a deobfuscated/cracked script.

have fun, take care and enjoy exploiting.
]]

local UILibrary = {}
function UILibrary.Load(name)
    local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/shlexware/Orion/main/source')))()
local GameName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name
local Window = OrionLib:MakeWindow({IntroText = "Loading Voidz",Name = GameName.." - Voidz", HidePremium = false, SaveConfig = true, ConfigFolder = "Voidz_DaHood"})
...
OrionLib:MakeNotification({
	Name = "Voidz",
	Content = "this script will not be getting updated anytime soon. this script is a cracked version of Dimag X, but i rebranded it into a voidz script cause bored.",
...
function modl.AddPage(Name)
  ...
  local modl2 = {}
  function modl2.AddToggle(name,def,callback)
     Tab:AddToggle({
	Name = name,
	Default = def,
	Callback = callback    
...
function modl2.AddButton(name,callback)
      Tab:AddButton({
	Name = name,
	Callback = callback    
})
end
function modl2.FuckDimag()
Tab:AddParagraph("Voidz","Credits to Trade#6676 For creating this script.")
Tab:AddButton({Name="Copy Youtube Channel",Callback=function()setclipboard("https://www.youtube.com/channel/UCvwwKecs9_8dUrihKg-j5Ow")end})
Tab:AddButton({Name="Copy Website",Callback=function()setclipboard("https://rtrade.github.io")end})
    end
return modl2
...
local MainUI = UILibrary.Load("GET CRACKED DIMAG  BOZOOOO")
...
local FunstuffPage = MainUI.AddPage("Fun")
...
-- funtab 
local fuckdimag = FunstuffPage.AddButton("FOV [PRO]", function()
    workspace.CurrentCamera.FieldOfView = 120
end)

local no = FunstuffPage.AddButton("Mask Names", function()
...
local nope = FunstuffPage.AddButton("PP Bat", function()
...
local bikeflyer = FunstuffPage.AddButton("Bike Fly", function()
	local speed = 20
	local old
	local Plr = game.Players.LocalPlayer
	local wheels = {}
...
local chatlog = FunstuffPage.AddButton("Chat Logs", function()
    local LP = game.Players.LocalPlayer
